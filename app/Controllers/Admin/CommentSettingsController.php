 ...
 * 负责处理网站评论设置相关的请求
 */
class CommentSettingsController
{
    /**
     * 评论设置首页
     */
    public function index()
    {
        ...
        // 设置当前页面变量用于侧边栏高亮
        $sub = 'comment';
        $currentAction = 'comment_settings';
    ...
    public function save()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                error_log('CommentSettingsController config save DB: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['database' => '保存配置时发生数据库错误'];
            } catch (Exception $e) {
                error_log('CommentSettingsController config save: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['file' => '保存配置时发生文件错误'];
            }

            header('Location: admin.php?action=comment_settings');
            exit;
        }
    }
}
