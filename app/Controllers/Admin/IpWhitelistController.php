    ...
    public function save() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                error_log('IpWhitelistController config save DB: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['database' => '保存配置时发生数据库错误'];
            } catch (Exception $e) {
                error_log('IpWhitelistController config save: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['file' => '保存配置时发生文件错误'];
            }
            
            header('Location: admin.php?action=ip_whitelist');
            exit;
        }
    }
}
