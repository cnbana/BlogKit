 ...
 * 负责处理网站注册设置相关的请求
 */
class RegisterSettingsController
{
    /**
     * 注册设置首页
     */
    public function index()
    {
        ...
        // 设置当前页面变量用于侧边栏高亮
        $sub = 'register';
        $currentAction = 'register_settings';
    ...
    public function save()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                error_log('RegisterSettingsController config save DB: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['database' => '保存配置时发生数据库错误'];
            } catch (Exception $e) {
                error_log('RegisterSettingsController config save: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['file' => '保存配置时发生文件错误'];
            }

            header('Location: admin.php?action=register_settings');
            exit;
        }
    }
}
