 ...
 * 负责处理网站伪静态设置相关的请求
 */
class RewriteSettingsController
{
    /**
     * 伪静态设置首页
     */
    ...
    public function save()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                error_log('RewriteSettingsController config save DB: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['database' => '保存配置时发生数据库错误'];
            } catch (Exception $e) {
                error_log('RewriteSettingsController config save: ' . $e->getMessage());
                $_SESSION['config_errors'] = ['file' => '保存配置时发生文件错误'];
            }

            header('Location: admin.php?action=rewrite');
            exit;
        }
    }
}
